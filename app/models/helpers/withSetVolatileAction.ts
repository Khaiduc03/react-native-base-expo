import { Instance, IStateTreeNode, SnapshotOut } from "mobx-state-tree"
 ...
 */
export const withSetVolatileAction = <T extends IStateTreeNode>(mstInstance: T) => ({
  // generic setter for all properties
  setVolatile<K extends Exclude<keyof Instance<T>, keyof SnapshotOut<T>>, V extends Instance<T>[K]>(
    field: K,
    newValue: V,
  ) {
